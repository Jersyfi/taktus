...
| [`Autonomy.json`](Autonomy.json) | a process's autonomy with its reason (ADR-0026) | `level`, `reason`, and `toward_next` — required below level 4, absent at 4 |
| [`LedgerEntry.json`](LedgerEntry.json) | one link of the hash chain | references only, no payload; `prev_hash` null only for the first entry; `egress.*` kinds record what left the system (ADR-0022); `removal.tested` records a removal test's verdict for the adapter it names, with the result's digest; `refs.trace_id` names the trace the entry was recorded in, so that an entry and a trace can be joined |
