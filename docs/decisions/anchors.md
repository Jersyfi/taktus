...
| O4 | **Any change to the substance of an accepted ADR** | An *ADR* (architecture decision record, `docs/adr/`) states a decision, the alternatives rejected and the consequences. Changing what was decided is the owner's call. Correcting a defect in how it was written is not — see §2, D6. |
...
documentation the change touches. Raising one of these as a question to the owner is a defect in
...
| D6 | **Correcting a defect in the repository's own documents** — an ambiguity, a contradiction, a statement that turns out to be wrong | an ADR amendment where an ADR is involved, and a DEFECT record under `docs/decisions/` (ADR-0017 §2) |

D6 has one exception. If correcting the defect would change what the software actually does — not
...
4. If the question is "which of two readings of a document is right", stop: that is a defect (D6),
