...
environment variable `TAKTUS_DATABASE_URL`, read by the `alembic` command line through
...
def database_url() -> str:
    given = config.attributes.get("url") or os.environ.get("TAKTUS_DATABASE_URL")
    if not given:
        raise SystemExit(
            "no database configured: set TAKTUS_DATABASE_URL (see .env.example), or run the "
            "migrations from code with the url in config.attributes"
        )
