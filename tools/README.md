...
| `preflight.sh` | every make target, through `need-<tool>`; `make doctor` | a missing tool names itself, what it is for and the one command that installs it, instead of `make: uv: No such file or directory`; `make doctor` reports two levels — the system tools (`uv`, `git`, `gitleaks`; `docker` and `node` optional), and the project environment with the tools the gates actually invoke through `uv run` (ruff, mypy, pytest, lint-imports, taktusctl) — says which is incomplete, and exits non-zero if either is |
...
nothing is broken, not strict. `make gates` needs `uv`, `git` and `gitleaks` on the path, and nothing else
