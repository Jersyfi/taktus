...
with nothing but that file. `check_decisions.py`, `check_adrs.py` and `checkdocs.py` use the standard library only
and run the same way; CI runs the first three without `make install`.
