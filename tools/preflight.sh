...
purpose() {
    case "$1" in
        ...
        git) echo "version control; gate-docs (checkdocs.py) and the coding worker require it on the path" ;;
...
install_command() {
    case "$1" in
        ...
        git)
            if [ "$(uname -s)" = "Darwin" ]; then
                echo "brew install git (or: xcode-select --install)"
            else
                echo "see https://git-scm.com/downloads/linux (or your distribution's package)"
            fi
            ;;
        docker)
            if [ "$(uname -s)" = "Darwin" ]; then
